...
# Tests for HTTP/2 protocol with http proxy websockets support.
...
is(connect_status(port => 8082), 405, 'accept_extended_connect off');
...
sub websocket_connect {
	...
	$s->h2_body(Protocol::WebSocket::Frame->new($message)->to_bytes,
		{ body_more => 1 }) if defined $message;

	my $frames = $s->read(all => [{ sid => $sid, fin => 0x4 }]);
...
sub websocket_write {
	...
	# unlike HTTP/1.1, sending is bounded by flow control, and h2_body()
	# sends nothing at all if the windows are too small; wait for nginx
	# to replenish them, keeping the data that arrives meanwhile

	for (1 .. 100) {
		last if length($buf) <= $s->{conn_window}
			&& length($buf) <= $s->{streams}{$c->{sid}};

		my $frames = $s->read(all => [{ type => 'WINDOW_UPDATE' }]);

		websocket_recv($c, $frames);
	}

	$s->h2_body($buf, { body_more => 1 });
}
...
sub connect_headers {
	...
	return [
		{ name => ':method', value => $method, mode => 1 },
		{ name => ':protocol', value => $protocol, mode => 2 },
		{ name => ':scheme', value => 'http', mode => 1 },
		{ name => ':path', value => $path, mode => 1 },
		{ name => ':authority', value => 'localhost', mode => 1 },
		{ name => 'sec-websocket-version', value => '13', mode => 2 },
		{ name => 'sec-websocket-key', value => 'ignored', mode => 2 }];
}
...
sub connect_response {
	...
	my $s = Test::Nginx::HTTP2->new($extra{port} ? port($extra{port}) : ());
