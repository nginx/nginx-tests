...
my $t = Test::Nginx->new()->has(qw/http http_v2/)->plan(15)
...
http {
    ...
    server {
        ...
        location /partial_i/ {
            ...
            # through the merge so parsing of the client value is observable.
            add_header Priority "i";
            return 200 "partial priority i";
        }
    }
}
...
$sid = $s->new_stream({ headers => [
	{ name => ':method', value => 'GET', mode => 0 },
	{ name => ':scheme', value => 'http', mode => 0 },
	{ name => ':path', value => '/proxy_partial_i/', mode => 1 },
	{ name => ':authority', value => 'localhost', mode => 1 },
	{ name => 'priority', value => 'u=5', mode => 2 }]});
h2_priority_update($s, $sid, 'a=1');
$frames = $s->read(all => [{ sid => $sid, fin => 1 }]);
...
# RFC9218 / RFC8941: a comma inside a quoted string is not a member separator.
# The client sends u=5 plus an unknown member x="a,u=0"; the embedded ",u=0"
# must not be mistaken for a top-level urgency member.  The upstream sets only
# "i" (no urgency), so the client urgency shows through the merge: the result
# must be "u=5, i".  A value of "u=0, i" would prove the quoted comma was
# mis-split and the spurious "u=0" wrongly applied.
...
$sid = $s->new_stream({ headers => [
	...
	{ name => ':path', value => '/proxy_partial_i/', mode => 1 },
	{ name => ':authority', value => 'localhost', mode => 1 },
	{ name => 'priority', value => 'u=5, x="a,u=0"', mode => 2 }]});
$frames = $s->read(all => [{ sid => $sid, fin => 1 }]);

($headers) = grep { $_->{type} eq 'HEADERS' } @$frames;
is($headers->{headers}{priority}, 'u=5, i',
	'comma inside quoted string is not a member separator');
