...
# Tests for HTTP/2 proxy backend.
...
my $t = Test::Nginx->new()->has(qw/http rewrite http_v2 proxy/)
	->has(qw/upstream_keepalive/)->plan(167);
...
http {
    ...
    server {
        ...
        location /LongHeader {
            ...
            proxy_set_header TE "trailers";
            proxy_pass_trailers on;
            proxy_set_header X-LongHeader $arg_h;
        }
...
is($frame->{flags}, 4, 'GET request - HEADERS flags');
ok((my $sid = $frame->{sid}) % 2, 'GET request - HEADERS sid odd');
is($frame->{headers}{':method'}, 'GET', 'GET request - method');
is($frame->{headers}{':scheme'}, 'http', 'GET request - scheme');
is($frame->{headers}{':path'}, '/', 'GET request - path');
is($frame->{headers}{':authority'}, "127.0.0.1:$p", 'GET request - authority');
is($frame->{headers}{te}, 'trailers', 'GET request - te');

$frames = $f->{http_end}();
($frame) = grep { $_->{type} eq "HEADERS" } @$frames;
is($frame->{flags}, 4, 'GET response - HEADERS flags');
is($frame->{sid}, 1, 'GET response - HEADERS sid');
is($frame->{headers}{':status'}, '200', 'GET response - status');
ok($frame->{headers}{server}, 'GET response - server');
ok($frame->{headers}{date}, 'GET response - date');
ok(my $c = $frame->{headers}{'x-connection'}, 'GET response - connection');

($frame) = grep { $_->{type} eq "DATA" } @$frames;
is($frame->{data}, 'Hello world', 'GET response - DATA');
is($frame->{length}, 11, 'GET response - DATA length');
is($frame->{flags}, 0, 'GET response - DATA flags');
is($frame->{sid}, 1, 'GET response - DATA sid');

(undef, $frame) = grep { $_->{type} eq "HEADERS" } @$frames;
is($frame->{flags}, 5, 'GET response - trailers flags');
is($frame->{sid}, 1, 'GET response - trailers sid');
is($frame->{headers}{'x-message'}, '', 'GET response - trailers message');
is($frame->{headers}{'x-status'}, '0', 'GET response - trailers status');

$frames = $f->{http_start}('/SayHello', method => 'POST');
($frame) = grep { $_->{type} eq "HEADERS" } @$frames;
is($frame->{flags}, 4, 'request - HEADERS flags');
ok(($sid = $frame->{sid}) % 2, 'request - HEADERS sid odd');
...
is($frame->{sid}, 3, 'response - HEADERS sid');
...
# upstream keepalive
# filter setting INITIAL_WINDOW_SIZE is inherited in the next stream

$f->{http_start}('/KeepAlive');
$f->{data}('Hello');
$f->{settings}(0, 1 => 4096);
$frames = $f->{http_end}(filter_settings => { 0x4 => 2 });
($frame) = grep { $_->{type} eq "HEADERS" } @$frames;
...
($frame) = grep { $_->{type} eq "DATA" } @$frames;
is($frame->{data}, 'He', 'filter setting - DATA');
is($frame->{length}, 2, 'filter setting - DATA length');
is($frame->{flags}, 0, 'filter setting - DATA flags');
$f->{settings}(0, 0x4 => 5);
...
# upstream keepalive - GOAWAY in filter, current stream aborted

$f->{http_start}('/KeepAlive');
$f->{data}('Hello');
$frames = $f->{http_end}(filter_goaway => [0, 0, 5]);
($frame) = grep { $_->{type} eq "HEADERS" } @$frames;
ok($c = $frame->{headers}{'x-connection'}, 'keepalive 6 - connection');
($frame) = grep { $_->{type} eq "RST_STREAM" } @$frames;
ok($frame, 'keepalive 6 - filter GOAWAY aborted stream');
...
# error response, no empty data frame expected

$f->{http_start}('/SayHello');
$f->{data}('Hello');
$frames = $f->{http_err}();
($frame) = grep { $_->{type} eq "HEADERS" } @$frames;
is($frame->{flags}, 5, 'error response - HEADERS flags');
($frame) = grep { $_->{type} eq "DATA" } @$frames;
ok(!$frame, 'error response - no DATA frame');
...
is($frame->{headers}{'x-message'}, '', 'continuation - trailers message');
is($frame->{headers}{'x-status'}, '0', 'continuation - trailers status');
...
# upstream keepalive, error response
# receiving END_STREAM followed by RST_STREAM NO_ERROR
...
($frame) = grep { $_->{type} eq "HEADERS" } @$frames;
ok($frame->{headers}{'x-status'}, 'keepalive 3 - error response, rst');
...
reused:
	...
	$f->{http_end} = sub {
		...
		$c->h2_settings(0, %{$extra{filter_settings}})
			if $extra{filter_settings};
		$c->h2_goaway(@{$extra{filter_goaway}})
			if $extra{filter_goaway};
	...
	$f->{http_err} = sub {
		$c->new_stream({ headers => [
			...
			{ name => 'content-type', value => 'text/plain',
				mode => 1, huff => 1 },
			{ name => 'x-status', value => '12',
				mode => 2, huff => 1 },
			{ name => 'x-message', value => 'unknown service',
				mode => 2, huff => 1 },
		]}, $sid);
